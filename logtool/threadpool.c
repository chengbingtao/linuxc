...
{
    struct my_threadpool *pool = NULL;
    do 
    {
        pool = malloc(sizeof(struct my_threadpool));
        if (NULL == pool)
        {
            printf("failed to malloc my_threadpool!\n");
            break;
        }
...
int threadpool_destroy(struct my_threadpool *pool)
{
    assert(pool != NULL);
    pthread_mutex_lock(&(pool->mutex));
